...
fnEval(expr){const e=expr.trim();const mm=e.match(/^([A-Z_][A-Z0-9_]*)\((.*)\)$/i);if(!mm)return null;const fn=mm[1].toUpperCase();const args=mm[2].length?mm[2].split(',').map(a=>this.evalExpr(a.trim())):[];const n=x=>this.num(x)||0;if(fn==='SIN')return Math.sin(n(args[0]));if(fn==='COS')return Math.cos(n(args[0]));if(fn==='TAN')return Math.tan(n(args[0]));if(fn==='ABS')return Math.abs(n(args[0]));if(fn==='MIN')return Math.min(...args.map(n));if(fn==='MAX')return Math.max(...args.map(n));if(fn==='LEN')return String(args[0]??'').length;if(fn==='UPPER')return String(args[0]??'').toUpperCase();if(fn==='LOWER')return String(args[0]??'').toLowerCase();if(fn==='SUBSTR')return String(args[0]??'').substring(n(args[1]),n(args[1])+n(args[2]));return null}
...
if(u.startsWith('DEREF ')){const m=t.match(/^DEREF\s+([A-Z_][A-Z0-9_]*)\s*,\s*([A-Z_][A-Z0-9_]*)$/i);if(!m)return this.err('EP119','DEREF',ins.line);const p=String(this.vars[m[1].toUpperCase()]||'');const k=p.startsWith('@')?p.slice(1):p;this.vars[m[2].toUpperCase()]=this.vars[k]??'';this.pc++;return}
if(u.startsWith('SETPTR ')){const m=t.match(/^SETPTR\s+([A-Z_][A-Z0-9_]*)\s*,\s*(.+)$/i);if(!m)return this.err('EP120','SETPTR',ins.line);const p=String(this.vars[m[1].toUpperCase()]||'');const k=p.startsWith('@')?p.slice(1):p;this.vars[k]=this.evalExpr(m[2]);this.pc++;return}
