...
fnEval(expr){const e=expr.trim();const mm=e.match(/^([A-Z_][A-Z0-9_]*)\((.*)\)$/i);if(!mm)return null;const fn=mm[1].toUpperCase();const args=mm[2].length?mm[2].split(',').map(a=>this.evalExpr(a.trim())):[];const n=x=>this.num(x)||0;if(fn==='SIN')return Math.sin(n(args[0]));if(fn==='COS')return Math.cos(n(args[0]));if(fn==='TAN')return Math.tan(n(args[0]));if(fn==='ABS')return Math.abs(n(args[0]));if(fn==='MIN')return Math.min(...args.map(n));if(fn==='MAX')return Math.max(...args.map(n));if(fn==='SQRT')return Math.sqrt(n(args[0]));if(fn==='POW')return Math.pow(n(args[0]),n(args[1]));if(fn==='LOG')return Math.log(n(args[0]));if(fn==='EXP')return Math.exp(n(args[0]));if(fn==='FLOOR')return Math.floor(n(args[0]));if(fn==='CEIL')return Math.ceil(n(args[0]));if(fn==='ROUND')return Math.round(n(args[0]));if(fn==='LEN')return String(args[0]??'').length;if(fn==='UPPER')return String(args[0]??'').toUpperCase();if(fn==='LOWER')return String(args[0]??'').toLowerCase();if(fn==='SUBSTR')return String(args[0]??'').substring(n(args[1]),n(args[1])+n(args[2]));if(fn==='ORD')return String(args[0]??'').charCodeAt(0)||0;if(fn==='CHR')return String.fromCharCode(n(args[0])&255);if(fn==='HEX')return (n(args[0])>>>0).toString(16).toUpperCase();if(fn==='BIN')return (n(args[0])>>>0).toString(2);if(fn==='UNHEX')return parseInt(String(args[0]||'0'),16)||0;if(fn==='UNBIN')return parseInt(String(args[0]||'0'),2)||0;return null}
numExpr(expr){if(/[\'"]/ .test(expr))return null;if(!/[+\-*/%()]/.test(expr))return null;const mapped=expr.replace(/[A-Z_][A-Z0-9_]*/gi,m=>{if(m.toUpperCase() in this.vars){const v=this.vars[m.toUpperCase()];return typeof v==='number'?String(v):'0'}return m});if(!/^[0-9+\-*/%().\s]+$/.test(mapped))return null;try{return Function('return ('+mapped+')')()}catch{return null}}

evalExpr(expr){expr=expr.trim();const fv=this.fnEval(expr);if(fv!==null)return fv;const ncalc=this.numExpr(expr);if(ncalc!==null&&Number.isFinite(ncalc))return ncalc;const add=expr.split('+').map(s=>s.trim());if(add.length>1){let acc=this.resolveToken(add[0]);for(let i=1;i<add.length;i++){const b=this.resolveToken(add[i]);if(typeof acc==='number'&&typeof b==='number')acc+=b;else acc=String(acc)+String(b)}return acc}return this.resolveToken(expr)}
...
stepOne(){if(this.pc>=this.prog.length){this.state='IDLE';this.io.ui.status('DONE');return}const ins=this.prog[this.pc];if(!ins._split&&ins.raw.includes(' : ')&&!/^LET\s+/i.test(ins.raw)){const parts=ins.raw.split(/\s:\s/).filter(Boolean);if(parts.length>1){this.prog.splice(this.pc,1,...parts.map(p=>({line:ins.line,raw:p,tokens:[]})));return}}const t=ins.raw,u=t.toUpperCase();if(this.trace)this.io.ui.row(10,`PC=${String(this.pc).padStart(3,'0')} ${u.slice(0,20)}`);
...
if(u.startsWith('WHILE ')){const m=t.match(/^WHILE\s+(.+)\s+DO\s+(@[A-Z0-9_]+)$/i);if(!m)return this.err('EP117','WHILE',ins.line);if(this.test(m[1],ins.line)){this.jump(m[2],ins.line);return}this.pc++;return}
if(u.startsWith('REF ')){const m=t.match(/^REF\s+([A-Z_][A-Z0-9_]*)\s*,\s*([A-Z_][A-Z0-9_]*)$/i);if(!m)return this.err('EP118','REF',ins.line);this.vars[m[1].toUpperCase()]='@'+m[2].toUpperCase();this.types[m[1].toUpperCase()]='STR';this.pc++;return}
if(u.startsWith('DEREF ')){const m=t.match(/^DEREF\s+([A-Z_][A-Z0-9_]*)\s*,\s*([A-Z_][A-Z0-9_]*)$/i);if(!m)return this.err('EP119','DEREF',ins.line);const p=String(this.vars[m[1].toUpperCase()]||'');const k=p.startsWith('@')?p.slice(1):p;if(k.includes(':')){const [a,ii]=k.split(':');const arr=Array.isArray(this.vars[a])?this.vars[a]:[];this.vars[m[2].toUpperCase()]=arr[(+ii)||0]??'';}else this.vars[m[2].toUpperCase()]=this.vars[k]??'';this.pc++;return}
if(u.startsWith('SETPTR ')){const m=t.match(/^SETPTR\s+([A-Z_][A-Z0-9_]*)\s*,\s*(.+)$/i);if(!m)return this.err('EP120','SETPTR',ins.line);const p=String(this.vars[m[1].toUpperCase()]||'');const k=p.startsWith('@')?p.slice(1):p;if(k.includes(':')){const [a,ii]=k.split(':');if(!Array.isArray(this.vars[a]))this.vars[a]=[];this.vars[a][(+ii)||0]=this.evalExpr(m[2]);}else this.vars[k]=this.evalExpr(m[2]);this.pc++;return}
if(u.startsWith('REFI ')){const m=t.match(/^REFI\s+([A-Z_][A-Z0-9_]*)\s*,\s*([A-Z_][A-Z0-9_]*)\s*,\s*(.+)$/i);if(!m)return this.err('EP121','REFI',ins.line);const idx=this.num(this.evalExpr(m[3]))||0;this.vars[m[1].toUpperCase()]='@'+m[2].toUpperCase()+':'+idx;this.types[m[1].toUpperCase()]='STR';this.pc++;return}
...
handleKey(e){const k=e.key||'';if(k==='Enter'){this.submitLine();return true}if(k==='Backspace'){this.backspace();return true}if(k==='Delete'){this.cli='';this.renderCli();return true}if(k==='Escape'){this.cli='';this.renderCli();return true}if(k==='Tab'){this.acceptChar(' ');this.acceptChar(' ');return true}if(k.length===1){const up=k.toUpperCase();if(this.vm&&this.vm.state==='RUNNING'&&this.vm.types&&this.vm.types.KEY==='STR'&&['1','-','0'].includes(up)){this.vm.vars.KEY=up;this.status('KEY '+up);return true}this.acceptChar(k);return true}return false}
