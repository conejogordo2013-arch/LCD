...
const tmp=path+'.tmp';this.db.files[tmp]=payload;this.db.meta[tmp]={size:payload.length,mtime:this.now(),type:'TMP',crc:this.crc16(payload),flags};
...
delete this.db.files[tmp];delete this.db.meta[tmp];this.appendJournal('WRITE',path,true);this.save();return {ok:true}}
...
castToType(type,val,line){if(type==='NUM'){const n=this.num(val);if(n===null)return this.err('ET200','NUM',line),null;return n}if(type==='BOOL')return val?1:0;if(type==='STATE'){const s=String(val).toUpperCase();if(!SYS.STATE.includes(s))return this.err('ET201','STATE',line),null;return s}return String(val).toUpperCase()}
...
if(u.startsWith('LET ')){const m=t.match(/^LET\s+([A-Z_][A-Z0-9_]*)\s*:\s*(NUM|STR|BOOL|STATE)\s*=\s*(.+)$/i);if(!m)return this.err('EP101','LET',ins.line);const k=m[1].toUpperCase(),ty=m[2].toUpperCase();const v=this.castToType(ty,this.evalExpr(m[3]),ins.line);if(this.state==='FAULT')return;this.types[k]=ty;this.vars[k]=v;this.pc++;return}
...
const display=new DisplayDriver(document.getElementById('grid'));const fs=new VFS();const parser=new Parser();const ui={print:()=>{},row:()=>{},status:()=>{}};const vm=new VM({ui,parser,fs});const term=new Terminal(display,fs,vm);ui.print=t=>term.print(t);ui.row=(y,t)=>term.row(y,t);ui.status=t=>term.status(t);
